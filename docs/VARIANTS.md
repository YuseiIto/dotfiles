...
* **Key Features:** Complete language runtimes, toolchains, LSPs, cloud/devops, security, and embedded compilers. Runtime `PATH` is exposed shell-agnostically via `~/.shell-env.sh` plus bash. Ships the shared, tool-agnostic agent guidance (the `agent-prompts` cookbook's SKILL.md skills and user preferences) into OpenHands' user-scope config — `~/.openhands/skills/` and `~/.openhands/microagents/` — so the same instructions used by Claude Code apply here too.

> **Note:** When OpenHands runs the agent as its own `openhands` user (UID 10001), toolchains installed under the build user's home are not on that user's `PATH`, and the agent guidance under the build user's `~/.openhands` is likewise not visible to it. Configure the OpenHands sandbox to run as the existing user / matching UID so both the installed toolchains and the provisioned agent config resolve correctly.
